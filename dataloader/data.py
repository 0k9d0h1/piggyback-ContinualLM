...
def get_dataset_t5(dataset_name,tokenizer,args):
    # --- Unsupervised Learning datasets ---
    # attributes: 'text'
    ...
    if dataset_name == 'restaurant_sup':
        ...
        for ds in ['train', 'test']:
            ...
            with open(os.path.join('./data/SemEval14-res', ds + '.json')) as f:
                data = json.load(f)
            for _data in data:
                new_data[ds]['text'].append(dataset_name + ' term: ' + data[_data]['term'] + ' sentence: ' + data[_data]['sentence'])  # add aspect as well
                new_data[ds]['labels'].append(label2idx(data[_data]['polarity']))
        datasets = DatasetDict(
            {
                'train': Dataset.from_dict(new_data['train']),
        ...
        for ds in ['train', 'test', 'dev']:
            ...
            with open('./data/chemprot/{}.jsonl'.format(f_ds), 'r+') as f:
                for item in jsonlines.Reader(f):
                    new_data[var_ds]['text'].append(dataset_name + ' ' + item['text'])
                    new_data[var_ds]['labels'].append(item['label'])
        ...
        for ds in ['train', 'test', 'dev']: #all train
            with open('./data/absa/dat/Bing5Domains/CanonG3/{}.json'.format(ds), 'r') as f:
                tmp_data = json.load(f)
            for dt in tmp_data:
                new_data['train']['labels'].append(tmp_data[dt]['polarity'])
                new_data['train']['text'].append(dataset_name + ' term: ' + tmp_data[dt]['term'] + ' sentence: ' + tmp_data[dt]['sentence'])
        ...
        for ds in ['train', 'test', 'dev']: # all test
            with open('./data/absa/dat/Bing5Domains/NikonCoolpix4300/{}.json'.format(ds), 'r') as f:
                tmp_data = json.load(f)
            for dt in tmp_data:
                new_data['test']['labels'].append(tmp_data[dt]['polarity'])
                new_data['test']['text'].append(dataset_name + ' term: ' + tmp_data[dt]['term'] + ' sentence: ' + tmp_data[dt]['sentence'])
