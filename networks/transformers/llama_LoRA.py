...
from .layers import LoRAPiggybackLinear, MultiTaskClassifier, PretrainingMultiTaskClassifier
...
class LoRALlamaMLP(nn.Module):
    def __init__(self, config):
        ...
        if 'gate_proj' in config.target_modules:
            self.gate_proj = LoRAPiggybackLinear(
                self.hidden_size, self.intermediate_size, config.lora_r, lora_alpha=config.lora_alpha, config=config, bias=False)
        else:
            self.gate_proj = nn.Linear(
                self.hidden_size, self.intermediate_size, bias=False)
        if 'up_proj' in config.target_modules:
            self.up_proj = LoRAPiggybackLinear(
                self.hidden_size, self.intermediate_size, config.lora_r, lora_alpha=config.lora_alpha, config=config, bias=False)
        else:
            self.up_proj = nn.Linear(
                self.hidden_size, self.intermediate_size, bias=False)
        if 'down_proj' in config.target_modules:
            self.down_proj = LoRAPiggybackLinear(
                self.intermediate_size, self.hidden_size, config.lora_r, lora_alpha=config.lora_alpha, config=config, bias=False)
        else:
    ...
    def forward(self, x, task_label):
        if self.config.pretraining_tp > 1:
            ...
            if 'gate_proj' in self.config.target_modules:
                gate = self.gate_proj(x, task_label)
            else:
                gate = self.gate_proj(x)
            if 'up_proj' in self.config.target_modules:
                up = self.up_proj(x, task_label)
            else:
                up = self.up_proj(x)
            if 'down_proj' in self.config.target_modules:
                down_proj = self.down_proj(self.act_fn(gate) * up, task_label)
            else:
...
class LoRALlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        if 'q_proj' in config.target_modules:
            self.q_proj = LoRAPiggybackLinear(
                self.hidden_size, self.num_heads * self.head_dim, config.lora_r, lora_alpha=config.lora_alpha, config=config, bias=config.attention_bias)
        else:
            self.q_proj = nn.Linear(
                self.hidden_size, self.num_heads * self.head_dim, bias=config.attention_bias)
        if 'k_proj' in config.target_modules:
            self.k_proj = LoRAPiggybackLinear(
                self.hidden_size, self.num_key_value_heads * self.head_dim, config.lora_r, lora_alpha=config.lora_alpha, config=config, bias=config.attention_bias)
        else:
            self.k_proj = nn.Linear(
                self.hidden_size, self.num_key_value_heads * self.head_dim, bias=config.attention_bias)
        if 'v_proj' in config.target_modules:
            self.v_proj = LoRAPiggybackLinear(
                self.hidden_size, self.num_key_value_heads * self.head_dim, config.lora_r, lora_alpha=config.lora_alpha, config=config, bias=config.attention_bias)
        else:
            self.v_proj = nn.Linear(
                self.hidden_size, self.num_key_value_heads * self.head_dim, bias=config.attention_bias)
        if 'o_proj' in config.target_modules:
            self.o_proj = LoRAPiggybackLinear(
                self.hidden_size, self.hidden_size, config.lora_r, lora_alpha=config.lora_alpha, config=config, bias=config.attention_bias)
        else:
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.config.pretraining_tp > 1:
            ...
            if 'q_proj' in self.config.target_modules:
                query_states = self.q_proj(hidden_states, task_label)
            else:
                query_states = self.q_proj(hidden_states)
            if 'k_proj' in self.config.target_modules:
                key_states = self.k_proj(hidden_states, task_label)
            else:
                key_states = self.k_proj(hidden_states)
            if 'v_proj' in self.config.target_modules:
        ...
        if self.config.pretraining_tp > 1:
            ...
            if 'o_proj' in self.config.target_modules:
...
class LoRALlamaFlashAttention2(LoRALlamaAttention):
    """
...
class LoRALlamaSdpaAttention(LoRALlamaAttention):
    """
...
LLAMA_ATTENTION_CLASSES = {
    "eager": LoRALlamaAttention,
    "flash_attention_2": LoRALlamaFlashAttention2,
    "sdpa": LoRALlamaSdpaAttention,
}


class LoRALlamaDecoderLayer(nn.Module):
...
class LoRALlamaModel(LlamaPreTrainedModel):
    """
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [LoRALlamaDecoderLayer(config, layer_idx)
...
class LoRALlamaForCausalLM(LlamaPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = LoRALlamaModel(config)
        self.vocab_size = config.vocab_size
        self.lm_head = PretrainingMultiTaskClassifier(
    ...
    def forward(
        ...
        if self.config.pretraining_tp > 1:
            ...
            logits = self.lm_head(hidden_states, task_label)
        logits = logits.float()
...
class LoRALlamaForSequenceClassification(LlamaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
